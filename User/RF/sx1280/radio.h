...
struct Radio_s
{
     ...
     */
    void ( *Init )( RadioCallbacks_t *callbacks1 ,RadioCallbacks_t *callbacks2 );
...
#define NONE 0
#define WORK 1
#define WAIT_REBOOT 2
#define REBOOT_ING 3
	uint32_t reboot_times;
	uint32_t rf_send_timeout_times;	
	uint32_t rf_send_error_times;
	uint32_t rf_rev_nothing_time;
 ...
 */
void OnRxError1( IrqErrorCode_t );
 ...
 * \brief Function executed on Radio Rx Error event
 */
void OnRxError2( IrqErrorCode_t );
...
extern uint16_t	iSend, iRev;

extern uint8_t	sendBuf[64];
extern uint8_t	revBuf[128];

extern void radio_init(void);
extern void Radio_Send( uint8_t *data, uint8_t length );
extern uint8_t Radio_SendData(uint8_t *buf, uint8_t size);
extern uint8_t Radio_ReceiveData(uint8_t *buffer);

extern void Radio_write_data( uint8_t *data, uint8_t rf ,uint8_t len);

extern void Radio_start_send( uint8_t rf );

extern void Radio_set_rf_ch(uint8_t rf,uint8_t ch);
	
extern	void etc_reboot_rf(uint8_t rf);
#endif // __RADIO_H__
