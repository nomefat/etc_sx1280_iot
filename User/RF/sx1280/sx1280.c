...
void SX1280_PRO(uint8_t rf);
/*!
 * \brief Holds a flag raised on radio interrupt
 */
static bool IrqState;

static RadioCallbacks_t* RadioCallbacks[2];
...
void SX1280Init( RadioCallbacks_t *callbacks1,RadioCallbacks_t *callbacks2  )
{
    RadioCallbacks[0] = callbacks1;
		RadioCallbacks[1] = callbacks2;
...
void SX1280SetTx( TickTime_t timeout )
{
    ...
    if( SX1280GetPacketType( ) == PACKET_TYPE_RANGING )
    {
		...
		if(radio_switch == RF1)
		{
			...
			HAL_GPIO_WritePin(RF1_TX_EN_GPIO_Port,RF1_TX_EN_Pin,GPIO_PIN_SET);
			HAL_GPIO_WritePin(RF1_RX_EN_GPIO_Port,RF1_RX_EN_Pin,GPIO_PIN_RESET);
		}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if(GPIO_Pin == RF1_DIO1_EXTI9_Pin)
	{
		
		SX1280_PRO(RF1);
	}
	else if(GPIO_Pin == RF2_DIO1_EXTI3_Pin)
	{
		SX1280_PRO(RF2);
	}
...
//				case MODE_RX:
						if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
						{
								...
								{
										if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->rxError != NULL ) )
										{
												RadioCallbacks[radio_switch]->rxError( IRQ_CRC_ERROR_CODE );
										}
								}
								else
								{
										if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->rxDone != NULL ) )
										{
												RadioCallbacks[radio_switch]->rxDone( );
										}
								}
						}
						if( ( irqRegs & IRQ_HEADER_VALID ) == IRQ_HEADER_VALID )
						{
								if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->rxHeaderDone != NULL ) )
								{
										RadioCallbacks[radio_switch]->rxHeaderDone( );
								}
						}
						if( ( irqRegs & IRQ_HEADER_ERROR ) == IRQ_HEADER_ERROR )
						{
								if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->rxError != NULL ) )
								{
										RadioCallbacks[radio_switch]->rxError( IRQ_HEADER_ERROR_CODE );
								}
						}
						if( ( irqRegs & IRQ_RX_TX_TIMEOUT ) == IRQ_RX_TX_TIMEOUT )
						{
								if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->rxTimeout != NULL ) )
								{
										RadioCallbacks[radio_switch]->rxTimeout( );
								}
						}
						if( ( irqRegs & IRQ_RANGING_SLAVE_REQUEST_DISCARDED ) == IRQ_RANGING_SLAVE_REQUEST_DISCARDED )
						{
								if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->rxError != NULL ) )
								{
										RadioCallbacks[radio_switch]->rxError( IRQ_RANGING_ON_LORA_ERROR_CODE );
								}
						}
//						break;
//				case MODE_TX:
						...
						{
								if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->txDone != NULL ) )
								{
										RadioCallbacks[radio_switch]->txDone( );
								}
						}
						if( ( irqRegs & IRQ_RX_TX_TIMEOUT ) == IRQ_RX_TX_TIMEOUT )
						{
								if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->txTimeout != NULL ) )
								{
										RadioCallbacks[radio_switch]->txTimeout( );
								}
						}
//						break;
//				case MODE_CAD:
						if( ( irqRegs & IRQ_CAD_DONE ) == IRQ_CAD_DONE )
						{
								...
								{
										if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->cadDone != NULL ) )
										{
												RadioCallbacks[radio_switch]->cadDone( true );
										}
								}
								else
								{
										if( ( RadioCallbacks[radio_switch] != NULL ) && ( RadioCallbacks[radio_switch]->cadDone != NULL ) )
										{
												RadioCallbacks[radio_switch]->cadDone( false );
										}
								}
						}
