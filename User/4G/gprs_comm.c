...
int gprs_print_rx_tx_data_enable = 1;   //使能gprs收发数据打印功能
		...
		if(gprs_stat.con_client[i].connect_ok ==1)                //判断是否联网状态
		{
			...
			{	
				...
				if(gprs_stat.now_who_working == -1) //没有链接正在发送数据  
				{
					...
					gprs_stat.con_client[i].send_no_timeout = slot_count;  //超时清零
					return;				
				}
				else if(gprs_stat.now_who_working == i)  //本链接正在使用发送功能
				{
					...
					{			
						if(slot_count - gprs_stat.con_client[i].send_no_timeout > 3*128)
						{
							...
							gprs_stat.con_client[0].connect_ok = 0;
							gprs_stat.con_client[1].connect_ok = 0;
							sprintf(debug_send_buff,"Clint_%d发送数据等待>超时 重启4G\r\n",i);
							copy_string_to_double_buff(debug_send_buff);	
			...
			if(gprs_stat.con_client[i].connect_ok !=1)                //判断是否联网状态
			{
				...
				if(gprs_stat.con_client[i].send_cmd_timeout == 0 && etc_sys_param.etc_ap_param.server_ip[i] != 0)
				{
					if(gprs_stat.con_client[i].send_cmd_times%5 == 0)
					{
						...
						gprs_at_tcp_conn(i,etc_sys_param.etc_ap_param.server_ip[i],etc_sys_param.etc_ap_param.server_port[i]);      //发送一次命令后  20秒超时后才可以再次发送
